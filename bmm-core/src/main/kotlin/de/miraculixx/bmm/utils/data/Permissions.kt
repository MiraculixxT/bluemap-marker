package de.miraculixx.bmm.utils.data

const val manageOwnMarkers = "bmarker.command.markers"
const val manageOthersMarkers = "$manageOwnMarkers.others"

const val manageOwnSets = "bmarker.command.sets"
const val manageOthersSets = "$manageOwnSets.others"

const val changeVisibility = "bmarker.command.visibility"
const val changeVisibilityOthers = "$changeVisibility.others"

const val manageTemplates = "bmarker.command.templates"

const val manageSettings = "bmarker.command.settings"
